...
## Two parallel APIs — pick the right one

Both APIs implement the same R23 7.125-8.4 GHz macro reference (peak
78.3 dBm / 100 MHz = 46.1 dBm txPower + 32.2 dBi peak gain), but live in
different naming conventions and serve different purposes:
...
struct used by the `imtAas*` deterministic-grid functions).
...
## Streaming-only invariant (do not violate)

The Monte Carlo path **never materializes the per-draw `Naz x Nel x numMc`
EIRP cube**. `update_eirp_histograms` keeps a fixed-size aggregator only:
...
`eirp_exceedance_maps`, `export_eirp_percentile_table`,
`run_embrss_eirp_cdf_grid`) all operate on this aggregator. **Do not add code
paths that reconstruct the raw EIRP cube** — for the default 65,341-cell grid
with `numMc = 1e4` the cube would be ~5.2 GiB. Use `estimate_aas_mc_memory`
...
`embrss_aas_config` exposes a `powerMode` switch precisely to avoid this:
`'conducted'` takes `txPower_dBm` at face value; `'peak_eirp'` back-computes
conducted power from a target peak EIRP. Passing `peakEirp_dBm` while
`powerMode='conducted'` is rejected. Preserve that guard if you touch the
config builder.
